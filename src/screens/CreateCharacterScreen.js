...
export default function CreateCharacterScreen({ navigation, route }) {
  ...
  // v5.3.47 - Génère TOUJOURS une description basée sur les champs actuels
  const autoGenerateDescription = () => {
    const genderLabel = gender === 'female' ? 'Femme' : 'Homme';
    
    // Construire la description avec les valeurs actuelles ou par défaut
    ...
    desc += `yeux ${eyeColor || 'marron'}, `;
    desc += `peau ${skinTone || 'claire'}`;
