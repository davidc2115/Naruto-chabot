...
export default function CreateCharacterScreen({ navigation, route }) {
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        // v5.3.45 - Auto-générer la description physique si vide
        autoGenerateDescription();
        
        Alert.alert(
          '✅ Image importée', 
          'L\'image a été ajoutée et la description physique a été générée automatiquement.',
          [{ text: 'OK' }]
        );
      }
    } catch (error) {
  ...
  const takePhoto = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        Alert.alert(
          '✅ Photo prise', 
          'La photo a été ajoutée et la description physique a été générée automatiquement.',
          [{ text: 'OK' }]
        );
      }
    } catch (error) {
  ...
  // v5.3.45 - Appelé automatiquement quand une image est importée
  const autoGenerateDescription = () => {
    // Ne générer que si les champs physiques sont remplis
    if (!age) return;
    ...
    if (height) desc += `, ${height}cm`;
    if (bodyType) desc += `, silhouette ${bodyType}`;
    
    if (gender === 'female' && bust) {
      desc += `, poitrine bonnet ${bust}`;
    } else if (gender === 'male' && penis) {
      desc += `, ${penis}cm`;
    }
    ...
    if (hairLength || hairColor) {
      desc += `Cheveux ${hairLength || ''} ${hairColor || 'naturels'}`.trim() + ', ';
    }
    if (eyeColor) desc += `yeux ${eyeColor}, `;
    if (skinTone) desc += `peau ${skinTone}`;
    
    desc = desc.replace(/,\s*$/, '').trim() + '.';
    
    // Ne mettre à jour que si appearance est vide
