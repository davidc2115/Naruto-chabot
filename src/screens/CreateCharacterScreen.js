...
export default function CreateCharacterScreen({ navigation, route }) {
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        // v5.3.49 - Auto-générer la description physique basée sur les champs
        autoGenerateDescription();
        
        Alert.alert(
          '✅ Image importée', 
          'Image ajoutée!\n\n⚠️ La description physique est générée à partir des CHAMPS du formulaire (couleur cheveux, yeux, etc.).\n\nModifiez ces champs pour correspondre à votre image, puis appuyez sur "🔄 Générer".',
          [{ text: 'Compris!' }]
        );
      }
    } catch (error) {
  ...
  const takePhoto = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        Alert.alert(
          '✅ Photo prise', 
          'Photo ajoutée!\n\n⚠️ La description physique est générée à partir des CHAMPS du formulaire (couleur cheveux, yeux, etc.).\n\nModifiez ces champs pour correspondre à votre photo, puis appuyez sur "🔄 Générer".',
          [{ text: 'Compris!' }]
        );
      }
    } catch (error) {
  ...
  // === AUTO-GÉNÉRATION DE LA DESCRIPTION PHYSIQUE ===
  // v5.3.49 - Génère une description détaillée basée sur les champs du formulaire
  // IMPORTANT: L'app ne peut pas analyser l'image - la description vient des champs!
  const autoGenerateDescription = () => {
    const genderLabel = gender === 'female' ? 'Femme' : 'Homme';
    const genderAdj = gender === 'female' ? 'e' : '';
    
    // Construire une description TRÈS détaillée
    ...
    // Attributs spécifiques au genre
    if (gender === 'female') {
      ...
      desc += `, virilité de ${penis || '17'}cm`;
    }
    
    desc += '. ';
    
    // Détails du visage et cheveux avec plus de précision
    const hairColorFinal = hairColor || 'bruns';
    const hairLengthFinal = hairLength || 'longs';
    desc += `Cheveux ${hairLengthFinal} ${hairColorFinal}`;
    
    // Yeux avec expression
    const eyeColorFinal = eyeColor || 'marron';
    desc += `, yeux ${eyeColorFinal} expressifs`;
    
    // Teint de peau
    const skinToneFinal = skinTone || 'claire';
    desc += `, teint de peau ${skinToneFinal}`;
    
    desc = desc.trim() + '.';
    
    // TOUJOURS mettre à jour le champ appearance
    setAppearance(desc);
